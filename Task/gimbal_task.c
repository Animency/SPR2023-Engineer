...
extern float target_can2_203_angle;
  ...
  {	
		...
		if (toe_is_error(DBUS_TOE))
    {
      CAN_cmd_chassis(0,0,0,0);
		...
		gimbal_control.horizontal_scroll_motor[7].give_current,gimbal_control.horizontal_scroll_motor[3].give_current); //gimbal_control .horizontal_scroll_motor .give_current
  ...
  for (i=0;i<8;++i)
	{
		...
		if(i	==	7)
		{
			gimbal_move_update->horizontal_scroll_motor[i] .motor_speed_current  = MOTOR_RPM_TO_SPEED_2006 * gimbal_move_update ->horizontal_scroll_motor[i] .gimbal_motor_measure->speed_rpm;
		}
	}
...
static void gimbal_3508_control_loop(gimbal_control_t *gimbal_move_control_loop,int16_t i,float current_target_transfer)
{
  ...
  //计算pid
	...
	//	pid_test_1=gimbal_move_control_loop->horizontal_scroll_motor[7].motor_speed_current;
...
static void gimbal_3508_angle_control_loop(gimbal_control_t *gimbal_move_control_loop,int16_t i,int angle_target_transfer)
{
  ...
  //计算pid
	...
	pid_test_1 = gimbal_move_control_loop->horizontal_scroll_motor[7].gimbal_motor_measure->angle;
	//功率控制
  // chassis_power_control(chassis_move_control_loop);
...
//		 __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_4, duoji);
}
//底盘运动时的云台保持
static void gimbal_keep(gimbal_control_t *gimbal_keep)
{
...
static void gimbal_pid_init(gimbal_control_t *gimbal_pid_init)
{
  ...
  PID_init(&gimbal_pid_init->horizontal_scroll_motor[1] .gimbal_motor_speed_pid ,3000.0		, 200.0	,135.0	, 8000.0	, 3000.0);
	...
	PID_init(&gimbal_pid_init->horizontal_scroll_motor[7] .gimbal_motor_speed_pid ,3500.0		, 200.0	,100.0	, 8000.0	, 3000.0);
	//云台电机角度环PID初始化
	...
	PID_init(&gimbal_pid_init->gimbal_6020_motor.gimbal_motor_angle_pid 					,0.12 	, 0.0		, 0.14	, 10.0	, 0.3	); //0.15
	...
	PID_init(&gimbal_pid_init->horizontal_scroll_motor[7] .gimbal_motor_angle_pid ,0.04		, 0.0		,0.0		, 5.0		, 0.0);

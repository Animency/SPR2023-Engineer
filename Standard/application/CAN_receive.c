...
#define get_motor_measure(ptr, data)                               \
 ...
 motor_measure_t motor_gimbal[5];  //云台电机
...
void Calc_3508_Angle(motor_measure_t *motor)
{
  ...
  motor->angle = motor->round * ANGLE_T + motor->last_ecd;
}
	...
	else if (hcan == &hcan2)
{
	switch (rx_header.StdId)
  {
		case CAN_3508_GIMBAL_TAISHENG_LEFT_ID:
  {
    get_motor_measure(&motor_gimbal[3], rx_data);
		Calc_3508_Angle(&motor_gimbal[3]);
    break;
  }
	case CAN_3508_GIMBAL_TAISHENG_RIGHT_ID:
  {

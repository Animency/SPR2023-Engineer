...
void USART6_IRQHandler(void)
{
    ...
    else if (USART6->SR & UART_FLAG_IDLE)
    {
        ...
        if ((hdma_usart6_rx.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            //get receive data length, length = set_data_length - remain_length
            //????????,?? = ???? - ????
            this_time_rx_len = NX_RX_BUF_LENGTH - hdma_usart6_rx.Instance->NDTR;

            //reset set_data_lenght
            //????????
            hdma_usart6_rx.Instance->NDTR = NX_RX_BUF_LENGTH;
            ...
            if (this_time_rx_len == NX_RECIVE_LEN)
            {
				      NX_DATA_RECIVE(nx_board_recive[0],nx_data); 
            }
        }
        else
        {
            ...
            if (this_time_rx_len == NX_RECIVE_LEN )
            {
				      NX_DATA_RECIVE(nx_board_recive[1],nx_data);
            }
        }
    }
}